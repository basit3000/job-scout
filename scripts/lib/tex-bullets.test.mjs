...
test('enrichProjectBullet adds at most one posting-named tag', () => {
  const project = { title: 'Sample League', tags: ['Python', 'FastAPI', 'Discord', 'Docker'] };
...
test('enrichProjectBullet does not invent tags the posting did not name', () => {
  const project = { title: 'Sample League', tags: ['Python', 'Discord'] };
...
test('enrichProjectsBody handles nested \\cvitem labels', () => {
  const body =
    '\\cvitem{\\href{https://example.com/league}{Sample League}}{Designed a FastAPI service on PostgreSQL.}';
  const { body: out, changed } = enrichProjectsBody(
    body,
    ['docker'],
    [{ title: 'Sample League', tags: ['Docker'] }],
  );
  assert.equal(changed, true);
